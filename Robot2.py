...
def generate_report(start_time):
    ...
    total_time = (end_time - start_time) / 1000  # Total time in seconds
    print("\nFinal Report:")
...
def main():
    ...
    while running:
        ...
        # Check if the robot is on an acid square, change the robot's color
        current_pos = path[current_step]
        if current_pos in acid_squares:
            # Change robot color to red if on an acid square
            robot_color = (255, 255, 0)
        else:
            robot_color = (0, 200, 0)  # Green if not on an acid square

        draw_robot(robot_color)  # Draw robot with the updated color
        ...
        pygame.display.flip()
        clock.tick(1)
    generate_report(start_time)  # Generate and print final report
